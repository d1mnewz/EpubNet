...
namespace EpubNet.Console
{
	public static class EpubReaderExtensions
	{
		...
		public static string ToPlainText(this EpubBook book)
		{
			...
			foreach (var epubTextContentFile in book.Content.Html)
				sb.AppendLine(GetContentAsPlainText(WebUtility.HtmlDecode(epubTextContentFile.Value.Content)));

			return sb.ToString();
		}


		private static string GetContentAsPlainText(string html)
		{
		...
		{
			return text is null ? null : Regex.Replace(text, @"</?(\w+|\s*!--)[^>]*>", " ", RegexOptions);
		}
