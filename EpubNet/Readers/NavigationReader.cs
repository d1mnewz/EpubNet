...
namespace EpubNet.Readers
{
	...
	internal static class NavigationReader
	{
		...
		private static EpubNavigationPoint ReadNavigationPoint(XElement navigationPointNode)
		{
			...
			if (!result.NavigationLabels.Any()) throw new Exception($"EPUB parsing error: navigation point {result.Id} should contain at least one navigation label.");

			if (result.Content is null) throw new Exception($"EPUB parsing error: navigation point {result.Id} should contain content.");
		...
		private static EpubNavigationLabel ReadNavigationLabel(XElement navigationLabelNode)
		{
			...
			if (navigationLabelTextNode is null) throw new Exception("Incorrect EPUB navigation label: label text element is missing.");
		...
		private static EpubNavigationPageTarget ReadNavigationPageTarget(XElement navigationPageTargetNode)
		{
			...
			if (result.Type is default) throw new Exception("Incorrect EPUB navigation page target: page target type is missing.");
